...
class TrailedEdgeSourcesTestCase(AlgorithmTestCase, lsst.utils.tests.TestCase):
    ...
    def testNanFlag(self):
        ...
        original_check_edge_function = task.plugins['ext_trailedSources_Naive'].check_edge
        # Used to simulate a trailed source where one of the coordinates is a
        # nan.

        def check_edge_mock(*args, **kwargs):
            measRecord = args[0]
            exposure = args[1]
            x0 = args[2]
            y0 = args[3]
            x1 = args[4]
            y1 = np.nan  # overriden to test NAN flagging
            length = args[6]
            measRecord['ext_trailedSources_Naive_y1'] = np.nan
            return original_check_edge_function(measRecord, exposure, x0, y0, x1, y1, length)

        patcher = patch(
            'lsst.meas.extensions.trailedSources.NaivePlugin.SingleFrameNaiveTrailPlugin.check_edge',
            side_effect=check_edge_mock)
        patcher.start()
        task.run(catalog, exposure)
        record = catalog[0]
