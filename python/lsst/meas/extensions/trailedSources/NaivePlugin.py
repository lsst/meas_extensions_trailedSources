...
class SingleFrameNaiveTrailPlugin(SingleFramePlugin):
    ...
    def measure(self, measRecord, exposure):
        ...
        self.check_edge(measRecord, exposure, x0, y0, x1, y1, length)
    ...
    def check_edge(self, measRecord, exposure, x0, y0, x1, y1, length):
        """ Set flags for edge pixels, off chip, and nan trail coordinates.
        ...
        if np.isnan(x_coords).any() or np.isnan(y_coords).any():
            ...
            if not (all(exposure.getBBox().beginX <= x <= exposure.getBBox().endX for x in x_coords)
                ...
                if x0 is not np.nan and y0 is not np.nan:
                    if exposure.mask[Point2I(int(x0), int(y0))] & exposure.mask.getPlaneBitMask('EDGE') != 0:
                        self.flagHandler.setValue(measRecord, self.EDGE.number, True)

                if x1 is not np.nan and y1 is not np.nan:
                    if exposure.mask[Point2I(int(x1), int(y1))] & exposure.mask.getPlaneBitMask('EDGE') != 0:
                        self.flagHandler.setValue(measRecord, self.EDGE.number, True)
        ...
        # as their location
            ...
            if exposure.mask[Point2I(int(x0), int(y0))] and exposure.mask[Point2I(int(x1), int(y1))]:
                if ((exposure.mask[Point2I(int(x0), int(y0))] & exposure.mask.getPlaneBitMask('EDGE') != 0)
                    ...
                    self.flagHandler.setValue(measRecord, self.EDGE.number,
                                              True)
